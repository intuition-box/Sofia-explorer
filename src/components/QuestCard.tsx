...
export default function QuestCard({ item, displayName, avatar, isPrivate }: QuestCardProps) {
  ...
  return (
    <Card
      className="qc-card p-4 flex flex-col gap-2"
      style={{ '--quest-color': color } as React.CSSProperties}
    >
      {/* Decorative oversized Sofia logo bleeding off the right edge */}
      <div className="qc-hero" aria-hidden="true">
        <SofiaMini color={color} />
      </div>
      ...
      <div className="flex flex-col gap-1">
        <span className="qc-eyebrow">Earned</span>
        <span className="qc-title">{item.title}</span>
      </div>
    </Card>
  )
}
